...
- Широкое меню аналитики группирует страницы по пользовательскому контексту; пункты содержат иконку, название и одну строку назначения. Оно открывается по hover, focus и click, поддерживает стрелки, `Home/End` и `Escape`, удерживается внутри viewport и складывает четыре колонки в две, затем в одну. Не добавляй поиск, уведомления, профиль или историю посещений без работающего сценария.
