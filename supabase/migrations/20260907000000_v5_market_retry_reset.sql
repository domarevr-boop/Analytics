...
begin
  ...
  v_next_status := case when v_source_exists then 'uploaded' else 'created' end;
...
revoke all on function public.v5_market_reset_staging(uuid) from public, anon;
grant execute on function public.v5_market_reset_staging(uuid) to authenticated;
