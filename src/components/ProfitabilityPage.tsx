...
export default function ProfitabilityPage(filterProps: FilterBarProps) {
  ...
  return <div className="ds-page profit-page profit-page-v2">
    <AnalyticsPageHeader eyebrow="Бизнес-метрики" title="Рентабельность" description="Финансовый результат по периодам, кабинетам, категориям, склейкам и товарам." actions={<aside className="profit-settings"><div><strong>Параметры расчёта</strong><small>{monthLabel(expenseMonth)} {expenseMonth.slice(0, 4)}</small></div><div className="profit-setting-values">{cabinets.map(cabinet => <label key={cabinet.id}><span>{cabinet.name}</span><b><input type="number" min="0" max="100" step="0.1" value={getExtraExpenses(expenseMonth)[cabinet.id] || 0} onChange={event => setExtraExpense(expenseMonth, cabinet.id, Number(event.target.value) || 0)} />%</b></label>)}</div></aside>} />

    <AnalyticsToolbar className="profit-toolbar" status={<div className="profit-month-navigation" aria-label="Выбор месяца"><button type="button" className="profit-month-arrow" aria-label="Предыдущий месяц" disabled={activeMonthIndex <= 0} onClick={() => selectMonth(availableMonths[activeMonthIndex - 1])}>‹</button><div className="profit-month-strip">{availableMonths.map(month => <button type="button" key={month} className={activeMonth === month ? 'active' : ''} aria-pressed={activeMonth === month} onClick={() => selectMonth(month)}>{monthLabel(month)}</button>)}</div><button type="button" className="profit-month-arrow" aria-label="Следующий месяц" disabled={activeMonthIndex < 0 || activeMonthIndex >= availableMonths.length - 1} onClick={() => selectMonth(availableMonths[activeMonthIndex + 1])}>›</button><strong className="profit-active-month">{monthTitle(activeMonth)}</strong></div>}>
      <div className="date-filters"><DateRangeFilter label="Период" value={period} onChange={next => { setPeriod(next); setExpenseMonth(monthOf(next.end)); }} maxDate={records.map(row => row.period_end).sort().at(-1) || period.end} /></div><FilterBar {...filterProps} period={period} variant="dashboard" />
    </AnalyticsToolbar>

    <section className="ds-kpi-grid profit-kpis">{kpis.map(kpi => { const positive = kpi.inverse ? kpi.delta <= 0 : kpi.delta >= 0; return <KpiTile key={kpi.label} label={kpi.label} value={kpi.value} delta={fmt1(Math.abs(kpi.delta))} deltaSuffix={kpi.points ? ' п.п.' : '%'} tone={kpi.delta === 0 ? 'neutral' : positive ? 'positive' : 'negative'} direction={kpi.delta > 0 ? 'up' : kpi.delta < 0 ? 'down' : 'flat'} details={kpi.sub} />; })}</section>

    <section className="profit-chart-grid"><AnalyticsPanel className="profit-panel"><PanelHeader eyebrow="Динамика" title="Рентабельность и чистая прибыль" description="Финансовый результат выбранного периода" controls={<SegmentedControl value={granularity} label="Гранулярность графика" options={[{ value: 'day', label: 'День' }, { value: 'week', label: 'Неделя' }, { value: 'month', label: 'Месяц' }]} onChange={setGranularity} />} /><ResponsiveContainer width="100%" height={270}><ComposedChart data={dailyData} margin={{ top: 14, right: 22, left: 4, bottom: 4 }}><CartesianGrid stroke="#E8EDF3" strokeDasharray="3 3" /><XAxis dataKey="date" tick={{ fontSize: 9 }} /><YAxis yAxisId="money" tickFormatter={value => `${Math.round(Number(value) / 1000)}к`} tick={{ fontSize: 9 }} /><YAxis yAxisId="rate" orientation="right" tickFormatter={value => `${fmt1(Number(value))}%`} tick={{ fontSize: 9 }} /><Tooltip formatter={(value, name) => [name === 'Чистая прибыль' ? `${fmt(Number(value || 0))} ₽` : `${fmt1(Number(value || 0))}%`, name]} /><Bar yAxisId="money" dataKey="netProfit" name="Чистая прибыль" fill="#78A9F7" radius={[3, 3, 0, 0]} maxBarSize={22} /><Line yAxisId="rate" dataKey="profitability" name="Рентабельность" stroke="#10A778" strokeWidth={2.2} dot={false} /><Line yAxisId="rate" dataKey="margin" name="Маржинальность" stroke="#8B5CF6" strokeWidth={1.8} dot={false} /></ComposedChart></ResponsiveContainer></AnalyticsPanel>
      <AnalyticsPanel className="profit-panel"><PanelHeader eyebrow="Структура" title="Формирование чистой прибыли" description="Вклад выручки и расходов" controls={<small className="profit-chart-period">{monthLabel(monthOf(period.end))}</small>} /><ResponsiveContainer width="100%" height={270}><BarChart data={waterfall} margin={{ top: 22, right: 12, left: 8, bottom: 4 }}><CartesianGrid stroke="#E8EDF3" strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" tick={{ fontSize: 9 }} /><YAxis tickFormatter={value => `${Math.round(Number(value) / 1_000_000)}м`} tick={{ fontSize: 9 }} /><Tooltip formatter={(value, name, item) => name === 'Значение' ? [`${fmt(Number(item.payload.label))} ₽`, 'Изменение'] : [value, name]} /><ReferenceLine y={0} stroke="#8391A3" /><Bar dataKey="base" stackId="waterfall" fill="transparent" /><Bar dataKey="value" name="Значение" stackId="waterfall" radius={[3, 3, 0, 0]}>{waterfall.map(item => <Cell key={item.name} fill={item.color} />)}</Bar></BarChart></ResponsiveContainer></AnalyticsPanel></section>

    <AnalyticsPanel density="data" className="profit-table-panel"><PanelHeader eyebrow="Детализация" title="Финансовый результат по структуре бизнеса" description="Иерархия кабинет → категория → склейка → артикул" controls={<div className="profit-status-tabs"><button className={status === 'all' ? 'active' : ''} onClick={() => setStatus('all')}>Все <b>{statusCounts.all}</b></button><button className={status === 'profitable' ? 'active' : ''} onClick={() => setStatus('profitable')}>Прибыльные <b>{statusCounts.profitable}</b></button><button className={status === 'loss' ? 'active' : ''} onClick={() => setStatus('loss')}>Убыточные <b>{statusCounts.loss}</b></button><button className={status === 'no-sales' ? 'active' : ''} onClick={() => setStatus('no-sales')}>Без продаж <b>{statusCounts.noSales}</b></button></div>} />
