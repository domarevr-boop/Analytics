...
export default function CompetitorsPage() {
  ...
  const initialPeriod = getLatestWeekPeriod(dates.at(-1) || '2026-08-31');
  ...
  const activeStart = dates.includes(start) ? start : getLatestWeekPeriod(dates.at(-1) || end).start;
  ...
  return <section className="competitors-page competitors-v2 overview-design-page analytics-page-shell ds-page">
    <AnalyticsPageHeader eyebrow="Аналитика › Конкуренты" title="Конкуренты" description="Сравнение брендов, карточек, поискового спроса и обеспеченности остатками в загруженном конкурентном срезе." actions={<button type="button" className="ds-button" onClick={() => setShowHelp(true)}>Справка</button>} />
    ...
    {activeTab === 'top-dynamics' ? <CompetitorTopDynamics positions={positions} /> : !funnel.length ? <EmptyState title="Загрузите файл конкурентов" description="На странице «Импорт» выберите один Excel-файл с четырьмя листами: воронка, запросы, склады и ежедневные позиции." /> : <>
      <AnalyticsToolbar className="competitors-toolbar"><DateRangeFilter label="Период" value={{ start: activeStart, end: activeEnd }} onChange={period => { setStart(period.start); setEnd(period.end); setTablePage(0); }} maxDate={dates.at(-1) || activeEnd} /><select value={brandFilter} onChange={event => { setBrandFilter(event.target.value); setTablePage(0); }}><option value="">Все бренды</option>{brandOptions.map(brand => <option key={brand}>{brand}</option>)}</select><label><span>⌕</span><input value={query} onChange={event => { setQuery(event.target.value); setTablePage(0); }} placeholder="Бренд, продавец или артикул" /></label><button type="button" className="ds-button" onClick={() => { const period = getLatestWeekPeriod(dates.at(-1) || activeEnd); setBrandFilter(''); setQuery(''); setStart(period.start); setEnd(period.end); setTablePage(0); }}>Сбросить</button></AnalyticsToolbar>
      <section className="competitors-kpis ds-kpi-grid">{kpis.map(kpi => { const mainChange = change(kpi.mainSeries, kpi.percentagePoints); const ownChange = change(kpi.ownSeries, kpi.percentagePoints); return <KpiTile key={kpi.label} className="competitor-kpi" label={kpi.label} value={kpi.main} delta={mainChange.label} deltaSuffix="" comparison="срез" tone={mainChange.value > 0 ? 'positive' : mainChange.value < 0 ? 'negative' : 'neutral'} visual={kpi.mainSeries.length > 1 ? <Spark values={kpi.mainSeries} /> : undefined} details={<div className="competitor-kpi-own"><span>Мы</span><b>{kpi.own}</b><em className={ownChange.value > 0 ? 'positive' : ownChange.value < 0 ? 'negative' : ''}>{ownChange.label}</em>{kpi.ownSeries.length > 1 && <Spark values={kpi.ownSeries} tone="own" />}</div>} />; })}</section>
