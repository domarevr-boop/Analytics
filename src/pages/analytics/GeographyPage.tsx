...
export default function GeographyPage() {
  ...
  const previousFiltered = useMemo(() => records.filter(record => {
    ...
    if (region && record.region !== region) return false;
    return allowedProductIds.has(record.product_id);
  }), [records, allowedProductIds, comparisonPeriod, region]);
  ...
  const localityPlan = plan?.local_share_target ?? null;
  const localityPlanDelta = localityPlan === null ? null : totals.localShare - localityPlan;
  const localityProgress = localityPlan ? Math.min(100, totals.localShare / localityPlan * 100) : 0;
  ...
  return <section className="geo-page geo-page-v2">
    ...
    <div className="geo-content-grid">
      <article className="geo-card geo-insights-card"><div className="geo-card-head"><div><h2>Главные выводы</h2></div></div><div className="geo-insights"><div><i>◉</i><p><b>Локальность {localityPlan !== null && totals.localShare >= localityPlan ? 'выше цели' : 'требует внимания'}</b><span>{formatNumber(totals.localShare)}%{localityPlan !== null ? ` при плане ${formatNumber(localityPlan)}%` : ''}</span></p></div><div><i>◷</i><p><b>Среднее время доставки {deliveryDelta <= 0 ? 'снизилось' : 'выросло'}</b><span>{deliveryDelta > 0 ? '+' : ''}{formatNumber(deliveryDelta)} ч к прошлому периоду</span></p></div><div><i>⇄</i><p><b>Потенциал локализации</b><span>{formatNumber(potential)} заказов можно перевести в локальные</span></p></div><div><i>⌂</i><p><b>Регион роста</b><span>{growthRegion?.region || 'Нет данных'} имеет наибольший потенциал</span></p></div></div></article>
      <article className="geo-card geo-chart-card"><div className="geo-card-head"><div><h2>Динамика по дням</h2><p>Показатели выбранного географического среза</p></div><div className="geo-chart-controls"><select value={metric} onChange={event => setMetric(event.target.value as ChartMetric)}>{Object.entries(metricLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><div className="geo-segmented"><button className={granularity === 'day' ? 'active' : ''} onClick={() => setGranularity('day')}>День</button><button className={granularity === 'week' ? 'active' : ''} onClick={() => setGranularity('week')}>Неделя</button></div></div></div>
        <div className="geo-chart"><ResponsiveContainer width="100%" height={290}><LineChart data={chartData}><CartesianGrid stroke="#E8EEF5" strokeDasharray="3 3" /><XAxis dataKey="date" tickLine={false} axisLine={false}/><YAxis tickLine={false} axisLine={false}/><Tooltip formatter={value => formatNumber(Number(value || 0))} /><Legend />{metric === 'orders' ? <><Line dataKey="localOrders" name="Локальные заказы" stroke="#10B981" strokeWidth={2.4} dot={{ r: 2 }} /><Line dataKey="orders" name="Все заказы" stroke="#3B82F6" strokeWidth={2.4} dot={{ r: 2 }} /></> : <Line dataKey="value" name={metricLabels[metric]} stroke="#3B82F6" strokeWidth={2.5} dot={{ r: 2 }} />}</LineChart></ResponsiveContainer></div>
      </article>
    </div>
    <div className="geo-locality-grid">
      <article className="geo-card geo-map-card"><div className="geo-card-head"><div><h2>Локальность по регионам</h2><p>Доля локальных заказов, %</p></div></div><div className="geo-map-stage">{regionRows.slice(0, 9).map((row, index) => <button type="button" key={row.region} style={{ '--geo-strength': Math.max(.1, row.localShare / 100), '--geo-order': index } as React.CSSProperties} onClick={() => setSelectedGeo({ level: 'district', district: row.region })}><span>{row.region}</span><b>{formatNumber(row.localShare)}%</b></button>)}</div><div className="geo-map-legend"><span><i className="high" />&gt; 50%</span><span><i className="medium" />30–50%</span><span><i className="low" />10–30%</span><span><i className="zero" />&lt; 10%</span></div></article>
      <article className="geo-card geo-plan-card"><h2>План месяца</h2><p>{month || 'Выберите период'}</p><label>Цель по локальности<input type="number" min="0" max="100" value={plan?.local_share_target ?? ''} onChange={event => upsertGeographyPlan(month, event.target.value === '' ? null : Number(event.target.value), plan?.delivery_hours_target ?? null)} /></label><div className="geo-plan-fact"><span>Факт</span><strong>{formatNumber(totals.localShare)}%</strong><i><b style={{ width: `${localityProgress}%` }} /></i></div><div className="geo-plan-result"><span>Выполнение плана</span><strong className={localityPlanDelta !== null && localityPlanDelta >= 0 ? 'geo-positive' : 'geo-negative'}>{localityPlanDelta === null ? 'План не задан' : `${localityPlanDelta >= 0 ? '+' : ''}${formatNumber(localityPlanDelta)} п.п.`}</strong><small>{localityPlan ? `${formatNumber(localityProgress)}% от плана` : 'Задайте цель на месяц'}</small></div><label>Целевое СВД, ч<input type="number" min="0" value={plan?.delivery_hours_target ?? ''} onChange={event => upsertGeographyPlan(month, plan?.local_share_target ?? null, event.target.value === '' ? null : Number(event.target.value))} /></label></article>
    </div>
