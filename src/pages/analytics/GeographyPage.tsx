...
export default function GeographyPage() {
  ...
  const areas = useMemo(() => [...new Set(baseFiltered.filter(record => !region || record.region === region).map(record => record.area))].sort(), [baseFiltered, region]);
  const cities = useMemo(() => [...new Set(baseFiltered.filter(record => (!region || record.region === region) && (!area || record.area === area)).map(record => record.city))].sort(), [baseFiltered, region, area]);
  ...
  const detailAreas = useMemo(() => {
    ...
    filtered.forEach(record => {
      const key = `${record.region}|${record.area}`;
      const entry = recordsByArea.get(key) || { district: record.region, area: record.area, rows: [] };
    ...
    const rows = [...recordsByArea.entries()].map(([areaKey, entry]) => {
      ...
      entry.rows.forEach(record => appendToMap(recordsByCity, record.city, record));
