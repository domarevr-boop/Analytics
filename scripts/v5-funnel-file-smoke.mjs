...
import {
  buildFunnelStagedRows,
  extractFunnelWorkbook,
  FUNNEL_MAX_FILE_BYTES,
  splitFunnelRows,
} from '../src/features/funnel/funnelImportCore.ts';
import {
...
const fail = message => { console.error(`V5 funnel file smoke failed: ${message}`); process.exit(1); };
const sqlString = value => String(value).replaceAll("'", "''");

if (!inputPath) fail('pass an .xlsx report path and optional source (xway or wb_funnel) after --');
...
if (fileStats.size <= 0 || fileStats.size > FUNNEL_MAX_FILE_BYTES) fail('the control report size is outside the allowed range');
...
const smokeHash = createHash('sha256').update(fileBytes).update(`\0v5-funnel-file-smoke\0${source}`).digest('hex');
const fields = ['date', 'seller_sku', 'wb_sku', ...workbook.presentMetricFields];
const payloadSql = fields.flatMap((field, index) => [`'${field}'`, `item -> ${index + 1}`]).join(', ');
const integerFields = new Set(['impressions', 'clicks', 'carts', 'orders', 'ad_impressions', 'ad_clicks', 'ad_orders_qty']);
const amountFields = new Set(['ordered_amount', 'ad_ordered_amount', 'ad_spend']);
const allMetrics = [
  'impressions', 'clicks', 'carts', 'orders', 'ordered_amount',
  'ad_impressions', 'ad_clicks', 'ad_orders_qty', 'ad_ordered_amount', 'ad_spend',
];

function numericAssertion(field, value) {
  if (integerFields.has(field)) return `v_summary.${field} <> ${value}`;
  if (amountFields.has(field)) return `abs(v_summary.${field} - (${value})::numeric) > 0.000001`;
  return `v_summary.${field} <> 0`;
}
...
const routeChecks = routing.routes.map((route, routeIndex) => {
  ...
  const stagedRows = buildFunnelStagedRows(routeWorkbook);
  const productCount = new Set(routeWorkbook.rows.map(row => String(row.seller_sku || row.wb_sku || '').replace(/\.0+$/u, '').trim())).size;
  const totals = Object.fromEntries(allMetrics.map(field => [field, routeWorkbook.rows.reduce((sum, row) => (
    sum + (typeof row[field] === 'number' ? row[field] : 0)
  ), 0)]));
  const stageCalls = splitFunnelRows(stagedRows).map(chunk => {
    const compactRows = chunk.map(row => [row.row_number, ...fields.map(field => row.payload[field])]);
    return `perform public.v5_funnel_stage_rows(v_batch_id, (
  ...
  const summaryAssertions = allMetrics.map(field => numericAssertion(field, totals[field])).join('\n    or ');
  return {
    cabinet: route.cabinet,
    rowCount: stagedRows.length,
    productCount,
    sql: `
  v_created := public.v5_funnel_create_batch(
    '${source}', '${route.cabinet.id}'::uuid, '${sqlString(basename(inputPath))}',
  ...
  select * into v_summary from public.v5_funnel_summary(
    '${routeWorkbook.dateStart}'::date, '${routeWorkbook.dateEnd}'::date,
    array['${route.cabinet.id}'::uuid], null, null, null, null, null
  );
  if ${summaryAssertions}
    or v_summary.product_count <> ${productCount}
...
const cabinetSetup = routing.routes.map((route, index) => `
  ...
  values ('${route.cabinet.id}'::uuid, 'funnel-file-smoke-${index + 1}-${smokeHash.slice(0, 12)}', '${sqlString(route.cabinet.name)} smoke');`).join('\n');
...
begin
  ...
  if v_user_id is null then raise exception 'Funnel file smoke requires an existing V5 administrator'; end if;
...
const routeSummary = routeChecks
  ...
  .join('; ');
console.log(`Control ${source} report parsed: ${workbook.rows.length} rows across ${routing.routes.length} cabinet routes, ${workbook.dateStart} to ${workbook.dateEnd}.`);
...
const temporaryDirectory = await mkdtemp(join(tmpdir(), 'analytics-v5-funnel-'));
const sqlPath = join(temporaryDirectory, 'funnel-file-smoke.sql');
